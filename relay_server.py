...
# Fix Cloudflare blocking on cloud providers (Render, Railway, etc.)
# by creating a custom connector with proper settings
client = discord.Client(
    intents=intents,
    http_trace=None,
)

# Monkey-patch to fix Cloudflare issues on cloud hosts
_original_login = discord.http.HTTPClient.static_login

async def _patched_login(self, token):
    """Patch the HTTP client to use a proper User-Agent to avoid Cloudflare blocks."""
    ...
    self.token = token
    self._HTTPClient__session.headers["Authorization"] = f"Bot {token}"
    self._HTTPClient__session.headers["User-Agent"] = self.user_agent

    # Verify the token works
    data = await self.request(discord.http.Route("GET", "/users/@me"))
    return data

discord.http.HTTPClient.static_login = _patched_login
...
@client.event
async def on_ready():
    print(f"[BOT] Connected as {client.user} | Watching channel {CHANNEL_ID}", flush=True)
...
async def on_message(message):
    ...
    for d in results:
        detections.append(d)
        print(f"[DETECTION] #{d['id']} | {d['category']} | {d['brainrot_name']} | {d['value']}", flush=True)
