...
def parse_discord_message(msg):
    ...
    for i, embed in enumerate(msg.get("embeds", [])):
        ...
        # Get title from embed title + author name
        title = (embed.get("title") or "") + " " + (embed.get("author", {}).get("name") or "")
        title = title.strip()
        ...
        for field in embed.get("fields", []):
            ...
            if any(k in nl for k in ["brainrot", "name", "item", "pet"]):
                ...
                if not data["raw_server_id"]:
                    data["raw_server_id"] = val

        # Fallback brainrot name from description
        if not data["brainrot_name"] and embed.get("description"):
